...
const CHANGELOG = [
  {
    ...
    date: "2026-05-18",
    items: [
      "生徒画面で単元を選んで出題できる機能を追加",
      "先生画面の出題確認にも単元絞り込みを追加",
    ],
  },
  {
    version: "1.4.0",
    ...
    <div>
      <div style={styles.card}>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={styles.label}>出題する単元</label>
          <select style={styles.input} value={selectedUnit} onChange={(e) => setSelectedUnit(e.target.value)}>
...
export default function OnlineTutorQuizApp() {
  ...
  const units = useMemo(() => {
    return Array.from(new Set(questions.map((question) => question.unit || "未分類"))).sort();
  }, [questions]);

  const filteredQuestions = useMemo(() => {
    if (selectedUnit === "all") return questions;
    return questions.filter((question) => (question.unit || "未分類") === selectedUnit);
  ...
  const startQuiz = () => {
    const queue = shuffleArray(filteredQuestions);
    setQuizQueue(queue);
    setCurrentIndex(0);
    setSelectedChoice("");
    setAnswered(false);
...
");
  ...
  const deleteQuestion = async (questionId) => {
    ...
    const confirmMessage = "問題 " + questionId + " を削除します。" + (target ? "
